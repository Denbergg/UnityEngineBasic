...
public class Movement : MonoBehaviour
{
    ...
    /// (게임오브젝트가 비활성 된 채로 실행되면 해당 게임오브젝트는 인스턴스화 되지 않고, 따라서 스크립트 인스턴스도 인스턴스화 되지않는다)
    /// 스크립트인스턴스를 비활성화하겠다고 해 놓아도
    /// 게임오브젝트가 인스턴스화 될때 해당 스크립트 인스턴스도 로드되기 때문에 Awake는 호출된다.
    /// -> 생성자 대용으로 사용함 (생성자와 같다는것은 아님. 멤버변수들의 초기화를 해야할 때 주로 Awake에서 함)
    /// </summary>
    private void Awake()
    {
        Debug.Log("Awake");
    ...
    /// <summary>
    /// Awake 에서 초기화가 완료된 다른 참조들을 사용해서 멤버 또는 다른 객체 생성 등을 한번 수행해야한다고 할 때 주로 사용
    /// </summary>
    void Start()
    {
        Debug.Log("Start");
    ...
    private void FixedUpdate()
    {
        ...
        //_transform.position = _transform.position + new Vector3(_h, 0.0f, _v) * Time.fixedDeltaTime;
    ...
    /// 이 스크립트를 컴포넌트로 가지는 GameObject 가 Trigger 를 가지고 있던지, other 의 Gameobject 가 Trigger 를 가지고 있던지
    /// 둘 중 하나라도 RigidBody 를 가지고 있어야 물리적인 연산이 적용되므로 단순히 Trigger-Trigger 또는 Trigger-Collider 끼리의 충돌이 아니라
    /// 둘 중 하나라도 RigidBody 를 가지고 있고, 둘 중에 하나라도 Trigger 를 가지고 있어야한다.
    /// </summary>
    ...
    /// <summary>
    /// 매 프레임의 마지막에 호출됨
    /// 게임 로직과 직접적인 연관이 없고 그냥 프레임마다 호출이 되기만 하면 될 때 주로씀 (Camera)
    /// </summary>
    ...
    /// Gizmos 를 그릴 때 내것도 그리고 싶으면 이 이벤트함수에 구현하면 됨.
    /// </summary>
    ...
    /// <summary>
    /// Graphic User Interface 를 렌더링할 때 호출되는 이벤트함수.
    /// 에디터를 커스터마이징 하거나 드로우콜 등에 대한 성능체크를 할 때 등에 사용함.
    /// </summary>
    private void OnGUI()
    {
    ...
    /// <summary>
    /// 앱이 종료될 때 호출.
    /// 앱이 종료되려는데 오브젝트를 생성한다거나 하는것을 막아야할 때 등에 응용할 수 있음.
    /// </summary>
    ...
    /// <summary>
    /// 이 스크립트인스턴스를 컴포넌트로 가지는 GameObject 가 비활성화 되거나
    ...
    /// <summary>
    /// 이 스크립트인스턴스를 컴포넌트로 가지는 GameObject 가 파괴되거나
