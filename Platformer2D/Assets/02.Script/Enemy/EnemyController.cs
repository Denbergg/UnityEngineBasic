...
public class EnemyController : MonoBehaviour
{
    ...
    private IEnumerator<int> currentWorkflow;
    // 어떤 상태일 때 어떤 로직을 수행해야 하는지에 대한 사전
    private Dictionary<StateType, IEnumerator<int>> _workflows = new Dictionary<StateType, IEnumerator<int>>();
    // 어떤 조건일 때 어떤 상태를 수행할 수 있는지에 대한 사전
    ...
    private void Update()
    {
        if (currentWorkflow.MoveNext() == false)
            currentWorkflow.Reset();
    }
